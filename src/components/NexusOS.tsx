...
const SceneCard = ({ scene, onActivate }) => {
  ...
  return (
    <button
      onClick={() => { setActive(true); onActivate(scene); setTimeout(() => setActive(false), 2000); }}
...
const HomeModule = ({ entities, onToggle, onActivateScene, areas }) => {
  const [selectedArea, setSelectedArea] = useState("all");
  const [scenes, setScenes] = useState(MOCK_SCENES);

  const toggleEntity = onToggle;
  const displayAreas = areas || MOCK_AREAS;

  // Fetch real scenes from HA
  useEffect(() => {
    fetch("/api/ha/scenes")
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        if (data?.configured && data.scenes?.length > 0) {
          const sceneIcons = [SunMedium, Video, Moon, Shield, Eye, Wind, Flame, Lightbulb];
          setScenes(data.scenes.map((s, i) => ({
            id: s.id,
            name: s.name,
            icon: sceneIcons[i % sceneIcons.length],
            description: "",
  ...
  // Pull real temps from entities if available
  const indoorTemp = entities.find((e) => e.domain === "sensor" && e.attributes?.device_class === "temperature" && !e.entity_id.includes("outdoor"));
  const outdoorTemp = entities.find((e) => e.domain === "sensor" && (e.entity_id.includes("outdoor") || e.entity_id.includes("outside")));
  const indoorValue = indoorTemp ? indoorTemp.state : "74.2";
  const outdoorValue = outdoorTemp ? outdoorTemp.state : "101.4";
  const tempUnit = indoorTemp?.attributes?.unit_of_measurement || "°F";
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-6 flex-wrap">
        ...
        <StatCard label="Indoor Temp" value={indoorValue} unit={tempUnit} icon={Thermometer} color={T.accent.blue} />
        <StatCard label="Outdoor" value={outdoorValue} unit={tempUnit} icon={SunMedium} color={T.accent.orange} />
        <StatCard label="Security" value={allLocked ? "Secured" : "Check"} icon={Shield} color={allLocked ? T.accent.green : T.accent.red} />
      </div>

      {/* Quick Scenes */}
      <div>
        <SectionHeader title="Quick Actions" subtitle={scenes[0]?.entity_id ? "Live HA scenes" : "Configure scenes in HA"} />
        <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
          {scenes.map((scene) => (
            <SceneCard key={scene.id} scene={scene} onActivate={onActivateScene || ((s) => console.log("Activate:", s?.id))} />
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex gap-2 mb-4 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
          ...
          {displayAreas.map((area) => (
            ...
            >
              {area.icon ? <area.icon className="w-3 h-3" /> : null}
              {area.name}
            </button>
          ))}
        </div>
      ...
      {controllable.length > 0 && (
        <div>
          <SectionHeader title="Devices" subtitle={`${controllable.length} in ${selectedArea === "all" ? "all rooms" : displayAreas.find((a) => a.id === selectedArea)?.name || selectedArea}`} />
...
const EnergyModule = () => {
  ...
  useEffect(() => {
    fetch("/api/ha/energy")
      .then((r) => r.json())
      .then((data) => {
        if (data.source === "live" && data.entities) setLiveEnergy(data);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  // Build chart data from live or mock
  const chartData = useMemo(() => {
    if (liveEnergy?.history) {
      return liveEnergy.history.slice(-24).map((h, i) => ({
  ...
  // Build device list from live or mock
  const devices = useMemo(() => {
    if (liveEnergy?.entities?.power?.length > 0) {
      const total = liveEnergy.entities.power.reduce((s, e) => s + e.state, 0) || 1;
      const colors = [T.accent.blue, T.accent.cyan, T.accent.orange, T.accent.yellow, T.accent.purple, T.accent.green];
      const icons = [Thermometer, Droplets, Flame, Lightbulb, Plug, Zap];
      return liveEnergy.entities.power.slice(0, 6).map((e, i) => ({
        name: e.name || e.entity_id,
        kwh: +(e.state / 1000).toFixed(2),
        pct: +((e.state / total) * 100).toFixed(1),
  ...
  // Stats
  const stats = useMemo(() => {
    if (liveEnergy?.entities) {
      const consumption = liveEnergy.entities.consumption.reduce((s, e) => s + e.state, 0);
      const solar = liveEnergy.entities.solar.reduce((s, e) => s + e.state, 0);
      const grid = Math.max(0, consumption - solar);
      return {
        consumption: consumption.toFixed(1),
        solar: solar.toFixed(1),
        grid: grid.toFixed(1),
        cost: (grid * 0.12).toFixed(2),
        source: "live",
      };
    }
    const c = ENERGY_DATA.reduce((s, d) => s + d.consumption, 0);
    const sol = ENERGY_DATA.reduce((s, d) => s + d.solar, 0);
    const g = Math.max(0, c - sol);
    return { consumption: c.toFixed(1), solar: sol.toFixed(1), grid: g.toFixed(1), cost: (g * 0.12).toFixed(2), source: "mock" };
  }, [liveEnergy]);
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Total Consumption" value={stats.consumption} unit="kWh" icon={Zap} color={T.accent.blue} />
        <StatCard label="Solar Production" value={stats.solar} unit="kWh" icon={SunMedium} color={T.accent.yellow} />
        <StatCard label="Grid Import" value={stats.grid} unit="kWh" icon={Plug} color={T.accent.orange} />
        <StatCard label="Estimated Cost" value={`$${stats.cost}`} icon={TrendingDown} color={T.accent.green} />
      </div>

      {/* Energy Chart */}
      <Card className="p-5">
        <SectionHeader title="Today's Energy Flow" subtitle={stats.source === "live" ? "Live HA data" : "Mock data — connect energy sensors"} />
...
const CamerasModule = () => {
  ...
  const [frigateConnected, setFrigateConnected] = useState(false);

  useEffect(() => {
    // Try Frigate API
    Promise.all([
      fetch("/api/ha/cameras?action=stats").then((r) => r.ok ? r.json() : null),
      fetch("/api/ha/cameras?action=events&limit=10").then((r) => r.ok ? r.json() : null),
    ]).then(([stats, evts]) => {
      if (stats?.cameras) {
        const cams = Object.entries(stats.cameras).map(([name, data]) => ({
          id: name,
        ...
        setCameras(cams);
        setFrigateConnected(true);
      }
      if (Array.isArray(evts) && evts.length > 0) {
        const formatted = evts.map((e) => ({
          time: new Date(e.start_time * 1000).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }),
          cam: e.camera.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
          type: e.label.charAt(0).toUpperCase() + e.label.slice(1),
          confidence: Math.round(e.top_score * 100),
          color: e.label === "person" ? T.accent.purple : e.label === "car" ? T.accent.blue : T.accent.yellow,
          id: e.id,
        }));
        setEvents(formatted);
        // Update detection counts on cameras
        setCameras((prev) => prev.map((cam) => ({
          ...cam,
          detections: evts.filter((e) => e.camera === cam.id).length,
          lastEvent: formatted.find((f) => f.cam.toLowerCase().replace(/ /g, "_") === cam.id)
            ? `${formatted.find((f) => f.cam.toLowerCase().replace(/ /g, "_") === cam.id).type} — ${formatted.find((f) => f.cam.toLowerCase().replace(/ /g, "_") === cam.id).time}`
            : "No events",
        })));
      }
    }).catch(() => {});
  ...
  const totalDetections = cameras.reduce((s, c) => s + c.detections, 0);

  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-2">
        <StatCard label="Cameras Online" value={`${cameras.length}/${cameras.length}`} icon={Cctv} color={T.accent.green} />
        <StatCard label="Detections Today" value={String(totalDetections)} icon={Eye} color={T.accent.purple} />
        <StatCard label="Source" value={frigateConnected ? "Frigate" : "Mock"} icon={Database} color={frigateConnected ? T.accent.green : T.accent.yellow} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {cameras.map((cam) => (
          <Card
            ...
            style={{ borderColor: selectedCam === cam.id ? T.accent.blue + "40" : T.border.subtle }}
          >
            <div
            ...
            >
              {frigateConnected && cam.hasLive ? (
                <img
                  src={`/api/ha/cameras?action=snapshot&camera=${cam.id}`}
                  alt={cam.name}
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <span className="text-[10px]" style={{ color: T.text.tertiary }}>Set FRIGATE_URL for live feeds</span>
                  </div>
                </div>
              )}
      ...
      <Card className="p-5">
        <SectionHeader title="Recent Events" subtitle={frigateConnected ? "Live Frigate events" : "Mock data"} />
        <div className="space-y-3">
          {displayEvents.map((event, i) => (
            <div key={event.id || i} className="flex items-center gap-3 py-2" style={{ borderBottom: i < displayEvents.length - 1 ? `1px solid ${T.border.subtle}` : "none" }}>
...
const AutomationsModule = () => {
  ...
  // Fetch real automations from HA
  useEffect(() => {
    fetch("/api/ha/automations")
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        if (data?.configured && data.automations?.length > 0) {
          setAutomations(data.automations.map((a) => ({
            id: a.entity_id,
            alias: a.alias,
            state: a.state,
  ...
  const toggleAutomation = async (id) => {
    if (liveConnected) {
      try {
        await fetch("/api/ha/automations", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ action: "toggle", entity_id: id }),
        });
        // Refresh
...
const SystemModule = ({ haConnected, haError, onRefresh }) => {
  const [haUrl, setHaUrl] = useState("");
  const [haToken, setHaToken] = useState("");
  const [connectionStatus, setConnectionStatus] = useState(haConnected ? "connected" : "disconnected");
  const [haConfig, setHaConfig] = useState(null);

  // Sync external connection state
  useEffect(() => {
    if (haConnected && connectionStatus !== "connected") {
      setConnectionStatus("connected");
      // Fetch HA config for version info
      fetch("/api/ha?action=config").then(r => r.json()).then(setHaConfig).catch(() => {});
    }
  }, [haConnected]);
  ...
  const testConnection = async () => {
    ...
    try {
      const res = await fetch("/api/ha?action=ping");
      const data = await res.json();
      if (data.connected) {
        setConnectionStatus("connected");
        const configRes = await fetch("/api/ha?action=config");
        const configData = await configRes.json();
        setHaConfig(configData);
  ...
  const haDetail = connectionStatus === "connected"
    ? `${haConfig?.version || "connected"} — ${haConfig?.location_name || "Home"}`
    : haError || "Not connected — set HA_URL and HA_TOKEN in .env.local";

  // Live health check data
  const [healthData, setHealthData] = useState(null);

  useEffect(() => {
    fetch("/api/ha/health").then(r => r.json()).then(setHealthData).catch(() => {});
  }, [connectionStatus]); // re-fetch when connection status changes

  const iconMap = { "Home Assistant": Home, "Frigate NVR": Cctv, "MQTT Broker": Router, "Claude API": Sparkles, "GitHub": GitBranch, "Vercel Deploy": Globe };
  const colorMap = { "Home Assistant": T.accent.blue, "Frigate NVR": T.accent.purple, "MQTT Broker": T.accent.green, "Claude API": T.accent.purple, "GitHub": T.text.secondary, "Vercel Deploy": T.text.secondary };
  ...
  const services = healthData?.services
    ? healthData.services.map((svc) => ({
        ...
        detail: svc.detail,
        icon: iconMap[svc.name] || CircleDot,
        color: colorMap[svc.name] || T.accent.blue,
        latency: svc.latency,
      }))
    : [
        { name: "Home Assistant", status: connectionStatus === "connected" ? "online" : "offline", detail: haDetail, icon: Home, color: T.accent.blue },
        { name: "Frigate NVR", status: "offline", detail: "Set FRIGATE_URL in .env.local", icon: Cctv, color: T.accent.purple },
        { name: "MQTT Broker", status: "offline", detail: "Set MQTT_URL in .env.local", icon: Router, color: T.accent.green },
        { name: "Claude API", status: "online", detail: "Sonnet 4 — Agent Bay ready", icon: Sparkles, color: T.accent.purple },
      ];
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl mx-auto">
      {/* HA Connection */}
      <Card className="p-5">
        <SectionHeader title="Home Assistant Connection" subtitle="Configured via .env.local (server-side)" />
        <div className="space-y-3">
          <div className="p-3 rounded-xl text-[11px] leading-relaxed" style={{ background: T.bg.input, color: T.text.secondary, border: `1px solid ${T.border.subtle}` }}>
            HA credentials are set in <code className="px-1 py-0.5 rounded" style={{ background: T.bg.elevated, fontFamily: T.font.mono, fontSize: 10 }}>.env.local</code> on the server for security. Set <code className="px-1 py-0.5 rounded" style={{ background: T.bg.elevated, fontFamily: T.font.mono, fontSize: 10 }}>HA_URL</code> and <code className="px-1 py-0.5 rounded" style={{ background: T.bg.elevated, fontFamily: T.font.mono, fontSize: 10 }}>HA_TOKEN</code>, then restart the dev server.
          </div>
          <button
      ...
      <Card className="p-5">
        <SectionHeader
          title="Services"
          subtitle={healthData ? `${healthData.status === "healthy" ? "All systems go" : "Some services need attention"} — ${healthData.total_latency_ms}ms` : "System health overview"}
          action={
            <button onClick={() => fetch("/api/ha/health").then(r => r.json()).then(setHealthData).catch(() => {})} className="text-[10px] px-2 py-1 rounded-lg hover:bg-white/5" style={{ color: T.text.tertiary }}>
              Refresh
            </button>
          }
        />
        <div className="space-y-2">
          {services.map((svc) => (
            <div key={svc.name} className="flex items-center gap-3 py-2.5" style={{ borderBottom: `1px solid ${T.border.subtle}` }}>
              ...
              {svc.latency && (
                <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: T.bg.elevated, color: T.text.tertiary, fontFamily: T.font.mono }}>{svc.latency}ms</span>
              )}
...
const AgentBay = ({ isOpen, onClose }) => {
  ...
  const send = () => {
    ...
    // Try real Agent Bay API first, fall back to mock if not configured
    (async () => {
      try {
        ...
        // Fallback: mock responses when API isn't configured yet
        const lower = userMsg.toLowerCase();
        let response = "Agent Bay API isn't configured yet. Add ANTHROPIC_API_KEY to .env.local, and optionally HA_URL + HA_TOKEN for home control. Once connected, I can control devices, check energy, manage cameras, and run automations.";
        if (lower.includes("light")) response = "With HA connected, I call light.turn_on or light.turn_off via the REST API. Add your HA_URL and HA_TOKEN in .env.local to go live.";
        if (lower.includes("lock")) response = "Security first. Once HA is connected, I call lock.lock on all your lock entities. Configure HA in .env.local to enable real device control.";
        if (lower.includes("temperature") || lower.includes("temp")) response = "Once HA is connected, I read climate entities in real-time and can adjust your thermostat. Mock data: 74.2°F indoor, 101.4°F outdoor, AC set to 72°F.";
        if (lower.includes("good night") || lower.includes("sleep")) response = "Good Night routine ready to wire up: lights off, doors locked, AC to 68°F, cameras armed. Connect HA to make it real.";
        if (lower.includes("energy") || lower.includes("power")) response = "Energy dashboard shows mock data. Connect HA energy sensors to see real consumption, solar production, and per-device breakdown.";
        if (lower.includes("camera") || lower.includes("detection")) response = "Camera grid shows placeholders. Connect Frigate (FRIGATE_URL in .env.local) for live feeds and AI detection events.";
        if (lower.includes("deploy")) response = "Add VERCEL_DEPLOY_HOOK to .env.local and I can trigger deploys. The MCP server at /api/mcp also exposes this for HA Assist voice commands.";
...
export default function NexusOS() {
  ...
  const [haStreaming, setHaStreaming] = useState(false);
  ...
  useEffect(() => {
    ...
    const transformEntity = (e) => ({
      ...
      area: e.attributes?.area_id || e.area || "unknown",
      state: e.state,
      ...
      try {
        ...
        source.onmessage = (event) => {
          try {
            ...
            if (data.type === "full") {
              setHaEntities(data.entities.map(transformEntity));
              setHaConnected(true);
              setHaStreaming(true);
              setHaError(null);
              retries = 0;
              ...
              setHaEntities((prev) => {
                const next = [...prev];
                data.entities.forEach((changed) => {
                  const idx = next.findIndex((e) => e.entity_id === changed.entity_id);
                  const transformed = transformEntity(changed);
              ...
              data.entities.forEach((e) => {
                ...
                if (e.entity_id.startsWith("binary_sensor.") && e.attributes?.device_class === "door" && e.state === "on") {
                  addNotification("info", `${e.attributes?.friendly_name || e.entity_id} opened`);
                }
              });
            }
          } catch {}
        ...
        source.onerror = () => {
          source.close();
          setHaStreaming(false);
          if (retries < 3) {
            retries++;
            setTimeout(connectStream, 2000 * retries);
          } else {
            startPolling();
          }
        };
    ...
    const startPolling = () => {
      const poll = async () => {
        try {
          ...
          const raw = Array.isArray(data) ? data : data.states || data.entities || [];
          if (raw.length > 0) {
            setHaEntities(raw.map(transformEntity));
            setHaConnected(true);
            setHaError(null);
          }
        } catch {
  ...
  const derivedAreas = useMemo(() => {
    ...
    entities.forEach((e) => {
      if (e.area && e.area !== "unknown" && !areaSet.has(e.area)) {
        areaSet.set(e.area, {
          id: e.area,
          name: e.area.split("_").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" "),
          icon: Home,
        });
      }
    });
    return areaSet.size > 0 ? Array.from(areaSet.values()) : MOCK_AREAS;
  }, [entities, haConnected]);
  ...
  const toggleHA = useCallback(async (entityId) => {
    ...
    try {
      ...
      addNotification("info", `Toggled ${entityId.split(".")[1].replace(/_/g, " ")}`);
    } catch (err) {
      addNotification("warning", `Toggle failed: ${err.message}`);
    }
  }, [haConnected, addNotification]);

  const activateScene = useCallback(async (scene) => {
    const entityId = scene?.entity_id || (scene?.id ? `scene.${scene.id}` : null);
    if (!entityId) return;
    if (!haConnected) {
      addNotification("info", `Scene "${scene.name || entityId}" activated (mock)`);
      return;
    }
    try {
      await fetch("/api/ha", {
      ...
      addNotification("info", `Scene activated`);
    } catch (err) {
      addNotification("warning", `Scene failed: ${err.message}`);
    }
  }, [haConnected, addNotification]);
  ...
  const renderModule = () => {
    switch (activeModule) {
      ...
      case "system": return <SystemModule haConnected={haConnected} haError={haError} onRefresh={fetchHAStates} />;
      default: return null;
    }
  };
        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2" style={{ maxWidth: 340 }}>
          {notifications.map((notif) => (
            <div
              ...
              style={{
                background: T.bg.base,
                border: `1px solid ${notif.type === "warning" ? T.accent.red + "40" : T.accent.blue + "30"}`,
                animation: "slideIn 0.3s ease-out",
              }}
            >
              <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${notif.type === "warning" ? T.accent.red : T.accent.blue}15` }}>
                {notif.type === "warning" ? (
                  <AlertTriangle className="w-3 h-3" style={{ color: T.accent.red }} />
                ) : (
                  <Info className="w-3 h-3" style={{ color: T.accent.blue }} />
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-medium truncate" style={{ color: T.text.primary }}>{notif.message}</div>
                <div className="text-[9px]" style={{ color: T.text.tertiary }}>{notif.time.toLocaleTimeString()}</div>
              </div>
              <button
                onClick={() => setNotifications((prev) => prev.filter((n) => n.id !== notif.id))}
                className="p-1 rounded hover:bg-white/5 flex-shrink-0"
              >
                <X className="w-3 h-3" style={{ color: T.text.tertiary }} />
              </button>
            </div>
          ))}
        </div>
